...
HAL_Init();
  ...
  while (1)
  {
	  ...
	  if ((S.CAN_MSG == START)|| (S.CAN_MSG == RESUME_AFTER_PAUSE)){
		  //FOR CYLINDER MOTORS WE WANT TO START OR PAUSE ONLY AFTER ITS COMPLETELY STOPPED.
		  //SO IF ITS RUNNING EVEN SLOWLY DISCARD THE START MSG
		  if(encSpeed.zeroSpeed == 1){
			  if (S.motorState == IDLE_STATE){
			...
			E.errorFlag=E.trackingError= 1; // NEXT TIME FIND A NEW ERROR CODE
			R.motorError|=1<<ERR_TE_SHIFT;
		  }
	  }
