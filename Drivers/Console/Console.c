	...
	while(1){
		...
		if (firstTime == 1){
			if (menuLevel == 0){
				...
				printf("\r\n NOTE: RAMPDOWN DISABLED FOR CYLINDERS");
				...
				if (C.runningOL == 0){
					if (C.OLmode == OL_TESTLOOP_MODE){
						if (noEntered == 1){
							...
							RM.rampRampUpTime_s = 60;
							RM.rampRampDownTime_s = 60;
						...
						if (noEntered == 4){
							...
							printf("\r\n Rotate Direction = %01d (CW-0,CCW-1) ",RM.rotateDirection);
							noEntered =0;
						}
					}
					if (C.OLmode == OL_FREERUN_MODE){
						if (C.OL_queryNo == 1){
							...
							if (noEntered == -1){
								...
								RM.rampRampUpTime_s = 60; // HARDCODED FOR CYLINDERs
